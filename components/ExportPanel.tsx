  ...
  return (
    <section className="fluent-card">
      ...
      <div className="export-grid">
        <span className="inline-flex w-full" title={!canExport ? "Please select and edit an image to export" : undefined} tabIndex={!canExport ? 0 : undefined}>
          <button className="primary-action w-full" disabled={!canExport} onClick={onExportPng}><Download size={16} />PNG</button>
        </span>
        <span className="inline-flex w-full" title={!canExport ? "Please select and edit an image to export as PDF" : undefined} tabIndex={!canExport ? 0 : undefined}>
          <button className="w-full" disabled={!canExport} onClick={onExportPdf}><FileDown size={16} />PDF</button>
        </span>
        <span className="inline-flex w-full" title={!canExport ? "Please select and edit an image to print" : undefined} tabIndex={!canExport ? 0 : undefined}>
