...
export function ExportPanel({ canExport, onExportPng, onExportPdf, onPrint }: ExportPanelProps) {
  const disabledTitle = !canExport ? "Upload a photo to export" : undefined;

  return (
    <section className="fluent-card">
      <div className="section-title"><span>Export</span></div>
      <div className="export-grid">
        <span title={disabledTitle} className="inline-flex w-full min-w-0" tabIndex={!canExport ? 0 : undefined}>
        ...
        <span title={!canExport ? "Upload a photo to print" : undefined} className="inline-flex w-full min-w-0" tabIndex={!canExport ? 0 : undefined}>
